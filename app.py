...
# --- 1. CONFIG & SYSTEM CSS ---
st.set_page_config(page_title="NBH Enterprise Portal", layout="wide")
...
st.markdown("""
    ...
    .nbh-off-label { margin: auto; font-size: 10px; color: #999; font-weight: bold; text-align: center; }
    </style>
...
def generate_slip_pdf(soc, month, name, p_df, net_val):
    ...
    pdf.set_font("Arial", "B", 11); pdf.cell(190, 10, f"Name: {name}", 0, 1); pdf.set_font("Arial", "B", 10); pdf.cell(190, 10, f"Final Net Payable: Rs. {net_val}", 1, 1); pdf.ln(5)
...
if not st.session_state.auth['logged_in']:
    ...
    if u_role == 'super_admin':
        ...
        conn = get_db(); all_socs = pd.read_sql("SELECT society_name, email FROM users WHERE role='manager'", conn); conn.close()
        st.dataframe(all_socs, use_container_width=True)
        with st.form("add_soc"):
            st.subheader("➕ Register New Society"); n, e, p = st.text_input("Name"), st.text_input("Email"), st.text_input("Pass")
            if st.form_submit_button("Create Account"):
                try: 
                    conn = get_db(); conn.execute("INSERT INTO users (email,password,society_name,role) VALUES (?,?,?,?)",(e,make_hash(p),n,'manager')); conn.commit(); conn.close(); st.success("Added!"); st.rerun()
        ...
        if page == "📅 Holiday Planner":
            ...
            if 'processed_data' not in st.session_state: st.warning("Process data in Dashboard first.")
                ...
                for n in m_data['Name'].unique():
                    ...
                    p, h, w, hl = len(e_df[e_df['Status'] == 'Present']), len(e_df[e_df['Status'] == 'Half Day']), len(e_df[e_df['Status'] == 'Weekly Off']), len(e_df[e_df['Status'] == 'Holiday'])
                    ot_p = round(e_df['OT_Hrs'].sum() * r['ot_rate'], 2); pens = len(e_df[e_df['Punctuality'] == 'Late']) * r['late_penalty']
                    if r['pay_type'] == "Monthly": base = round((r['base_salary'] / 30) * (p + w + hl + (h * r['half_day_rule'])), 2)
                    else: base = round(r['base_salary'] * (p + (h * 0.5)), 2)
                    # UNIFIED NAMING: 'Final Net Payable'
                    pay_rows.append({"Employee Name": n, "Salary Type": r['pay_type'], "Base Earned": base, "OT Pay": ot_p, "Final Net Payable": round(base + ot_p + r['bonus'] - pens, 2)})
                ...
                worker = st.selectbox("Select Worker for PDF Slip", pay_df['Employee Name'].unique())
            ...
            if f:
                ...
                conn = get_db()
                for n in df_raw['Name'].unique():
                    if not conn.execute('SELECT 1 FROM rosters WHERE employee_name=? AND society_email=?', (n, u_email)).fetchone():
                        conn.execute('INSERT INTO rosters (society_email, employee_name, category) VALUES (?,?,?)', (u_email, n, 'General'))
            ...
            if 'processed_data' in st.session_state:
                ...
                st.dataframe(m_data.groupby(['Name', 'Status']).size().unstack(fill_value=0).reset_index(), use_container_width=True)
                sel_n = st.selectbox("Individual Spotlight", m_data['Name'].unique()); p_df = m_data[m_data['Name'] == sel_n].sort_values('Date')
                ...
                for _, r in p_df.iterrows():
                    is_off_cls = "nbh-weekly-off" if r['Status'] == "Weekly Off" else ""
                    icon, color = mapping.get(r['Status'], ("❓", "#000"))
                    time_info = f"<span class='nbh-time-text'>{format_pretty_time(r['Worked_Hrs'])}</span>" if r['Worked_Hrs'] > 0 else ""
                    body = f"<div class='nbh-off-label'>{icon} Weekly Off</div>{time_info}" if r['Status'] == "Weekly Off" else f"<div class='nbh-status-box'><span class='nbh-status-icon'>{icon}</span><span class='nbh-status-label' style='color:{color};'>{r['Status']}</span>{time_info}</div>"
                    html.append(f'<div class="nbh-cal-day {is_off_cls}"><span class="nbh-day-num">{pd.to_datetime(r["Date"]).day}</span>{body}<div class="nbh-shift-footer">{r["In"]} - {r["Out"]}</div></div>')
